...
// Initialize branch indicator
async function initBranchIndicator() {
    try {
        ...
        const branchName = indicator.querySelector('.branch-name');

        // Update branch name
        branchName.textContent = data.branch;

        // Remove all branch type classes
        indicator.classList.remove('main', 'user', 'feature');

        // Add appropriate class based on branch type
        if (data.branch === 'main') {
            indicator.classList.add('main');
        } else if (data.branch.startsWith('user-')) {
            indicator.classList.add('user');
        } else if (data.branch.startsWith('feature-')) {
            indicator.classList.add('feature');
        }
    } catch (error) {
        console.error('Failed to fetch branch info:', error);
        document.getElementById('branchIndicator').querySelector('.branch-name').textContent = 'unknown';
    }
}
